...
public class BasicEntityBuilder {
	...
	public static IEntity createCube( double size, double centerX, double centerY, double centerZ ) {
		...
		// this.tetra.setPolygonColor( Color.RED );
	...
	public static IEntity createDiamond( Color color, double size, double centerX, double centerY, double centerZ ) {
		...
		int edges = 15;
		double inFactor = 0.8; // 80% the size of the outer ring, top ring points
		
		double halfSize = (size / 2);
		
		MyPoint bottom = new MyPoint( centerX, centerY, centerZ - halfSize );
		MyPoint[] outerPoints = new MyPoint[edges]; // outer points of diamond
		MyPoint[] innerPoints = new MyPoint[edges]; // inner points of diamond
		
		// create all points
		for (int i = 0; i < edges; i++) {
			double theta = 2 * (Math.PI / edges) * i; // specific spot for that point which scales with edge count
			
			double xPos = -Math.sin(theta) * halfSize; // must be inverted since its top-down view and the x is negative going towards the point of view
			double yPos = Math.cos(theta) * halfSize;
			double zPos = halfSize; // in the middle
		...
		MyPolygon polygons[] = new MyPolygon[ 1 + (edges * 2) ];
		for ( int i = 0; i < edges; i++ ) {
			polygons[i] = new MyPolygon( outerPoints[i], bottom, outerPoints[ (i+1) % edges ] ); // if it goes over the last number, it goes back to 1
		}
		for ( int i = 0; i < edges; i++ ) {
			polygons[i + edges] = new MyPolygon( outerPoints[i], outerPoints[ (i+1) % edges ], innerPoints[ (i+1) % edges], innerPoints[i] ); // if it goes over the last number, it goes back to 1
		}
	...
	public static IEntity createSphere( Color color, double diameter, int resolution, double centerX, double centerY, double centerZ ) {
